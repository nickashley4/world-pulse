...
// near-Earth asteroid flybys (NASA NeoWs), solar flares and Kp (NOAA SWPC). Falls back to the last
// good response per source when an API is down or rate-limited.
...
export async function loadSpace({ root, fetchJSON, etDay, days }) {
  ...
  let [prev, up, crew, apod, neo, flares, kp] = await Promise.all([
  ...
  return { apod: apodOut, launches, upcoming, crew: { count: people.length, people }, neos, flares: flaresOut, kp: kpOut };
}
