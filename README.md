...
- **Space mode:** NASA APOD & NeoWs (asteroid flybys), NOAA SWPC (solar flares, Kp, aurora forecast), The Space Devs Launch Library (launches, people in space), live ISS orbit propagated from its TLE, plus space news from NASA, ESA, SpaceNews, NASASpaceflight, Spaceflight Now, Space.com, Universe Today. Set `NASA_API_KEY` to avoid DEMO_KEY rate limits.
